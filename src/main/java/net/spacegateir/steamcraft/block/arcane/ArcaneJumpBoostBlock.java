...
public class ArcaneJumpBoostBlock extends Block {
    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        if (!world.isClient && entity instanceof LivingEntity livingEntity) {
            ...
            int baseAmplifier = (signalStrength == 0) ? 0 : 1;

            // Adjust duration and amplifier based on the redstone signal
            int adjustedDuration = baseDuration + (signalStrength * 600);
            int adjustedAmplifier = baseAmplifier + (signalStrength * 2);

            livingEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.JUMP_BOOST, adjustedDuration, adjustedAmplifier, false, false));
        }
