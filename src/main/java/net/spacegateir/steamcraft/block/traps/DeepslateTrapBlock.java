...
public class DeepslateTrapBlock extends Block {
    ...
    public DeepslateTrapBlock(Settings settings) {
        super(settings);
    }
    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        ...
        if (!world.isClient) {
            if (state.isOf(ModBlocks.DEEPSLATE_TRAP_BLOCK)) {
                // Change block to the spiked version
                world.setBlockState(pos, ModBlocks.DEEPSLATE_TRAP_BLOCK_SPIKES.getDefaultState());

                // Schedule the block to revert back to normal after 20 ticks
                world.scheduleBlockTick(pos, ModBlocks.DEEPSLATE_TRAP_BLOCK_SPIKES, 20);
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (state.isOf(ModBlocks.DEEPSLATE_TRAP_BLOCK_SPIKES)) {
            // Revert the block back to the normal trap block
            world.setBlockState(pos, ModBlocks.DEEPSLATE_TRAP_BLOCK.getDefaultState());
        }
    }
}
